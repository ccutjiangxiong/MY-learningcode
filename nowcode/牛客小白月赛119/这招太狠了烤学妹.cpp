...
void work() {
    ...
    auto f = [&](int y, int cuts) {
        int rem = y - cuts;
        if (rem <= 0) return 0LL;
        int p = cuts + 1;
        int a = rem / p, b = rem % p;
        return b * (a + 1) * (a + 2) / 2 + (p - b) * a * (a + 1) / 2;
    };
    ...
    auto cmp = [](const Node &A, const Node &B) { return A.delta < B.delta; };
    std::priority_queue<Node, vector<Node>, decltype(cmp)> pq(cmp);
    vector<int> t(m, 0);
    for (int i = 0; i < m; i++) {
        if (g[i] > 0) {
            int d = f(g[i], 0) - f(g[i], 1);
            pq.push({d, i});
        }
    }
    int ans = total;
