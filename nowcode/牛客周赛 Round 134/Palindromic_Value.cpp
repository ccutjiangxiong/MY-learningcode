...
void work() {
    ...
    cin >> n >> s;
    s = " " + s;
    vector<vi> pal(n + 1, vi(n + 1));
    rep(i, 1, n) {
        pal[i][i] = 1;
        if (i < n && s[i] == s[i + 1]) pal[i][i + 1] = 1;
    }
    rep(len, 3, n) rep(l, 1, n + 1 - len) {
        int r = l + len - 1;
        pal[l][r] = (pal[l + 1][r - 1] && s[l] == s[r]);
    }
    vi f(n + 1), g(n + 2);
    f[0] = g[n + 1] = 1;
    rep(i, 1, n) rep(j, 1, i) if (pal[j][i]) {
        f[i] += f[j - 1];
        f[i] %= MOD;
    }
    rem(i, n, 1) rep(j, i, n) {
        if (pal[i][j]) g[i] += g[j + 1], g[i] %= MOD;
    }
    int ans = 0;
    rep(l, 1, n) rep(r, 1, n) {
        if (pal[l][r])
            (ans += (r - l + 1) * (r - l + 1) % MOD * f[l - 1] % MOD * g[r + 1] % MOD) %= MOD;
    }
    pt(ans);
}
