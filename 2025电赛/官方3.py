...
def img_show(img):
    # 获取电池电量
    bat_percent = p.get_bat_percent()
    text = f"Battery: {bat_percent}%"
    img.draw_string(10, 10, text, color=image.COLOR_RED, thickness=2)
    fps = time.fps()
    img.draw_string(10, img.height() - 20, f"FPS: {fps:.2f}", color=image.COLOR_GREEN, thickness=2)
...
laser_switch = gpio.GPIO("GPIOA15", gpio.Mode.OUT)
def open_laser(state, laser):
    laser.value(state)
            ...
            if circle_dist > 0:
                ...
                if find_laser:
                    ...
                    if laser_blob:
                        ...
                        img.draw_circle(original_laser_point[0], original_laser_point[1], 4, image.COLOR_RED, 2)
    if original_center and original_center:
        uart_send(cal_dir(original_laser_point,original_center,laser_state))
    # disp.show(img)
    img_show(img)
    # print("图像中心:", center_pos)
    # print("检测圆心:", original_center)
    # print("偏差:", err_center)
