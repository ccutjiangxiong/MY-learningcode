...
find_laser = True           # 找激光点（未测试），实际使用时直接把摄像头中心和激光点保持移植就好了，不需要找激光点
...
while not app.need_exit():
                    ...
                    if circle_dist > 0:
                        ...
                        if (not find_circle) or (center_new):
                            ...
                            if find_laser:
                                ...
                                if laser_point:
                                    # 原图坐标
                                    points = np.array([[[laser_point.cx(), laser_point.cy()]]], dtype=np.float32)
                            ...
                            if DEBUG or debug_show_hires:
                                ...
                                if original_lasert_point is not None:
                                    img.draw_circle(original_lasert_point[0], original_lasert_point[1], 3, image.COLOR_RED, thickness=1)
                            else:
                            # 画在小图上显示
                                # too slow
                                ...
                                if original_lasert_point is not None:
                                    img_ai.draw_circle(original_lasert_point[0], original_lasert_point[1], 3, image.COLOR_RED, thickness=1)
    ...
    print("检测圆心:", original_center_point)
    print("偏差:", err_center)
