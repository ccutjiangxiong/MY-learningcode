...
void work() {
    ...
    cin >> N;
    long long half = N / 2;
    long long ceilHalf = (N + 1) / 2;
    long long pHalf = modPow(26, half);
    long long pCeil = modPow(26, ceilHalf);
    long long ans;
    if (N % 2 == 1) {
        // odd N: ans = N*26*26^{(N-1)/2} - 26^{(N+1)/2}
        ans = ((N % mod) * 26 % mod * pHalf % mod - pCeil + mod) % mod;
    } else {
        // even N: ans = 26^{N/2} * (N*13 - 1)
        long long t = ((N % mod) * 13 % mod - 1 + mod) % mod;
        ans = pCeil * t % mod;
    }
    cout << ans << "\n";
}
