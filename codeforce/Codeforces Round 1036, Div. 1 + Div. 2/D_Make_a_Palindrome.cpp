...
void work() {
    cin >> n >> k;
    ost<pii> tr;
    rep(i, 1, n) cin >> a[i], tr.insert({a[i], i});
    rep(i, 1, n) b[i] = 0;
    int x = tr.find_by_order(k - 1)->fi;
    vi g;
    rep(i, 1, n) if (a[i] <= x) g.pb(a[i]);
    int y = g.size() - k + 1;
    // for (int x : g) cout << x << ' ';
    // cout << endl;
    // pt(x);
    ...
    vi f;
    // pta(b, 1, 3);
    rep(i, 1, sz(g)) if (!b[i]) f.pb(g[i - 1]);
    // for (int x : f) cout << x << ' ';
    // cout << endl;
    // pt(fl, y);
    for (int i = 1; i * 2 <= sz(f); i++) {
        if (f[i - 1] != f[sz(f) - i]) fl = 0;
    }
