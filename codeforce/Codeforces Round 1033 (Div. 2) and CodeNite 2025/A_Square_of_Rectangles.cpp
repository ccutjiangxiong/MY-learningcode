...
void work() {
    ...
    vector<pair<ll, ll>> v{{l1, b1}, {l2, b2}, {l3, b3}};
    bool ok = false;
    vector<int> p{0, 1, 2};
    do {
        auto [w1, h1] = v[p[0]];
        ...
        if (h1 == h2) {
            ll S = w1 + w2;
            if (w3 == S && h1 + h3 == S) ok = true;
        }
        if (w1 == w2) {
            ll S = h1 + h2;
            if (h3 == S && w1 + w3 == S) ok = true;
        }
        if (h1 == h2 && h2 == h3) {
            ll S = h1;
            if (w1 + w2 + w3 == S) ok = true;
        }
        if (w1 == w2 && w2 == w3) {
            ll S = w1;
            if (h1 + h2 + h3 == S) ok = true;
        }
    } while (!ok && next_permutation(p.begin(), p.end()));
    // cout << Ye(ok) << endl;
    pt(Ye(ok));
}
