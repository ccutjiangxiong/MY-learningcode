...
void work() {
    ll n, m;
    cin >> n >> m;
    ll lo = n, hi = (ll)n * (n + 1) / 2;
    if (m < lo || m > hi) {
        cout << -1 << "\n";
        return;
    }
    auto S = [&](ll k) -> __int128 {
        return (__int128)k * (n - k + 1) + (__int128)(k - 1) * k / 2;
    };
    ll L = 1, R = n, k = n;
    while (L <= R) {
        ll mid = (L + R) >> 1;
    ...
    for (ll w = k + 1; w <= n; w++) {
        ll extra = min(rem, k - 1);
        rem -= extra;
        ll parent = extra + 1;  
        edges.emplace_back(parent, w);
    }
