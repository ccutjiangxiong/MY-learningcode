...
void work() {
    cin >> n;
    // 预处理：对每个素数索引 j 收集其所有 ≤n 的倍数
    vector<vi> mul(cnt + 1);
    rep(j, 1, cnt) {
        int pval = pri[j];
        if (pval > n) break;
        for (int x = pval; x <= n; x += pval) mul[j].pb(x);
    }
    int best_fixed = n + 1;
    ...
    rep(tr, 1, 200) {
        ...
        rep(j, 1, cnt) {
            ...
            shuffle(all(v), rng);
            for (int to : v) {
                if (to != pval && !used[to] && !used[pval]) {
                    swap(p[pval], p[to]);
                    used[pval] = used[to] = true;
                    break;
                }
            }
        }
        // 统计固定点
        int fixed = 0;
        rep(i, 1, n) if (p[i] == i) fixed++;
        if (fixed < best_fixed) {
            best_fixed = fixed;
            best_p = p;
            if (best_fixed <= 1) break;
        }
    }
    // 输出最优解
    rep(i, 1, n) { cout << best_p[i] << (i == n ? '\n' : ' '); }
}
signed main() {
    ios::sync_with_stdio(false);
    cin.tie(nullptr), cout.tie(nullptr);
    euler(N - 10);
    rep(i, 1, n) for (int j = 1; pri[j] <= i; j++) {
        if (i % pri[j] == 0) g[i].pb(j);
    }
    if (ti == 0) cin >> ti;
    while (ti--) {
        work();
