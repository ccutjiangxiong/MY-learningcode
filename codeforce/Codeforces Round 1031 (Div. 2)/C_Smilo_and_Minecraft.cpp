...
void work() {
    ...
    k--;
    ans = tot = 0;
    rep(i, 1, n) rep(j, 1, m) cin >> gr[i][j], pre[i][j] = pre[i][j - 1] + (gr[i][j] == 'g'),
                                               tot += gr[i][j] == 'g';
    // pt(pre[1][3], pre[2][3], pre[2][0]);
    rep(i, 1, n) rep(j, 1, m) {
        if (gr[i][j] == '.') {
            int res = 0;
            rep(x, max(1ll, i - k), min(i + k, n)) {
                int y1 = min(j + k, m), y2 = max(j - k - 1, 0ll);
                int u = (pre[x][y1] - pre[x][y2]);
                res += u;
                // pt(x, y1, y2, u);
            }
            ans = max(ans, tot - res);
