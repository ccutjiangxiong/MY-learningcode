...
void work() {
    cin >> n;
    // 1）读入 2n 张卡
    vector<ll> c(2 * n + 1), O(2 * n + 1), E(2 * n + 1);
    rep(i, 1, 2 * n) { cin >> c[i]; }
    // 2）算奇偶前缀和
    O[0] = E[0] = 0;
    rep(i, 1, 2 * n) {
    ...
    ll On = O[2 * n], En = E[2 * n];
    // 初始最小差
    ll best = llabs(On - En);
    // 分别维护 r 为奇/偶时可选的 val = 2*(E[r]-O[r])
    multiset<ll> S_odd, S_even;
    S_even.insert(0);  // l-1 = 0 时算在“偶”集合里

    rep(r, 1, 2 * n) {
        ll val = 2 * (E[r] - O[r]);
        ll cur = On - En + val;
        auto &S = (r & 1 ? S_odd : S_even);
        if (!S.empty()) {
            auto it = S.lower_bound(cur);
            if (it != S.end()) best = min(best, llabs(cur - *it));
            if (it != S.begin()) best = min(best, llabs(cur - *prev(it)));
        }
        S.insert(val);
    }
    // 最终最大化每位玩家能拿到的和 = (总和 - 最小差) / 2
    pt((On + En - best) / 2);
}
