...
int ti, n, m, k, a[N][111], b[N][111], c[111], ans, res, tot, x, y, z;
int f1[N], f2[N], g1[N], g2[N];
vi e[N];
// int gr[5555]
void work() {
    cin >> n >> m;
    vector<vi> gr(n + 1, vi(m + 1));
    rep(i, 1, n) rep(j, 1, m) cin >> gr[i][j], c[gr[i][j]]++;

    rep(i, 1, n) rep(j, 1, m) a[i][gr[i][j]]++, b[j][gr[i][j]]++;
    int ans = inf;
    rep(i, 1, n) rep(j, 1, m) {
        rep(u, 0, 100) c[u] -= a[i][u] + b[j][u];
        rep(u, 0, 100) c[u] += (a[i][u + 1] + b[j][u + 1]);
        c[gr[i][j]]++;
        c[gr[i][j] - 1]--;
        int res = 0;
        rem(u, 100, 0) if (c[u]) res = max(res, u);
        ans = min(res, ans);
        // pt(i, j, res);
