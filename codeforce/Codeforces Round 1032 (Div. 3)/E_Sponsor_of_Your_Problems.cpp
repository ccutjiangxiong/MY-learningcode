...
void work() {
    ...
    cin >> L >> R;
    int n = L.size();
    static int dp[15][2][2];

    rep(i, 0, n) rep(tl, 0, 1) rep(tr, 0, 1) dp[i][tl][tr] = inf;
    dp[0][1][1] = 0;
    rep(i, 0, n - 1) {
        rep(tl, 0, 1) rep(tr, 0, 1) {
            ...
            rep(d, lo, hi) {
                int ntl = tl && (d == lo);
                int ntr = tr && (d == hi);
                int cost = dp[i][tl][tr] + (d == L[i] - '0') + (d == R[i] - '0');
                dp[i + 1][ntl][ntr] = min(dp[i + 1][ntl][ntr], cost);
            }
        }
    }
    int ans = inf;
    rep(tl, 0, 1) rep(tr, 0, 1) ans = min(ans, dp[n][tl][tr]);
    cout << ans << "\n";
}
signed main() {
