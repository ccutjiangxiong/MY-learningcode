...
void init_fac(int n) {
    ...
    rem(i, n - 1, 0) ifac[i] = ifac[i + 1] * (i + 1) % MOD;
}
...
int det(vector<vector<int>> a) {
    ...
    rep(i, 0, n - 1) {
        ...
        rep(j, i + 1, n - 1) {
            int t = a[j][i] * inv % MOD;
            rep(k, i, n - 1) a[j][k] = (a[j][k] - t * a[i][k] % MOD + MOD) % MOD;
        }
    }
    return res;
}
...
void work() {
    ...
    rep(i, 0, k - 1) rep(j, 0, k - 1) { mat[i][j] = C(b[j + 1] - a[i + 1] + n - 1, n - 1); }
    cout << det(mat) << "\n";
}
