...
void work() {
    ...
    // 预处理 B 的积分：dB[i] = B[i-1] / i
    vector<mint> dB(n + 1);
    rep(i, 1, n) dB[i] = B[i - 1] / i;
    ...
    function<void(int, int)> cdq = [&](int l, int r) {
        ...
        // 取出 G[l..m-1]
        vmi segG(len);
        for (int i = l; i < m; i++) segG[i - l] = G[i];
        // 计算 exp(G) mod x^{len}
        vmi expG = Poly(segG).exp(len).a;
        // 取 A[0..len-1]
        vmi segA(len);
        rep(i, 0, len - 1) segA[i] = A[i];
        // 卷积 A * expG
        vmi conv = (Poly(segA) * Poly(expG)).a;

        // 对 [m..r) 做积分：G[i] += conv[i-l-1]/i + dB[i]
        for (int i = m; i < r; i++) {
            int k = i - l - 1;
            if (k >= 0 && k < (int)conv.size()) G[i] += conv[k] / i;
            G[i] += dB[i];
        }
