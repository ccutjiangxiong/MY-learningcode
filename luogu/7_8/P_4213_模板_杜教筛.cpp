...
int phi_g(int x) { return x; }
int phi_h(int x) { return (x + 1) * x / 2; }
int mu_h(int x) { return x = 1; }
// mu*1=e,phi*1=id,mu*id=phi
ll dujiao(ll n, auto Sg, auto Sh, auto& mp, int a[N]) {
    if (n < N) return a[n];
    if (mp.con(n)) return mp[n];
    ll res = Sh(n);
    for (ll l = 2, r; l <= n; l = r + 1) {
        r = n / (n / l);
        res -= (Sg(r) - Sg(l - 1)) * dujiao(n / l, Sg, Sh, mp, a);
    }
    return mp[n] = res / Sg(1);
}

void work() {
    cin >> n;
    int x = dujiao(n, phi_g, phi_h, mp1, phi);
    int y = dujiao(n, phi_g, mu_h, mp2, mu);
    pt(x, y);
}
// …existing code…
