...
void work() {
    ...
    sort(ord.begin(), ord.end(), [&](int u, int v) { return e[u].size() > e[v].size(); });

    vector<int> color(n + 1, 0);
    ans = n;  // upper bound = n

    function<void(int, int)> dfs = [&](int pos, int used) {
        if (used >= ans) return;  // prune
        if (pos == n) {           // all vertices colored
            ans = used;
            return;
        }
        int u = ord[pos];
        // mark colors used by neighbors
        vector<bool> usedc(used + 2, false);
        for (int v : e[u])
            if (color[v]) usedc[color[v]] = true;
        // try existing colors
