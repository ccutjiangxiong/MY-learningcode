...
int sm[N], pm[N], sv[N], pv[N];

static bool can(long long t) {
    long long total = 0;
    for (int i = 1; i <= n; i++) {
        long long need = max(0LL, a[i] * t - b[i]);
        if (!need) continue;
        long long best = LLONG_MAX;
        // try y big packs, y up to min(need/sv, sm)
        long long maxY = need / sv[i];
        long long lim = min(maxY, (long long)sm[i]);
        for (long long y = 0; y <= lim; y++) {
            long long rem = need - y * sv[i];
            if (rem < 0) rem = 0;
            long long x = (rem + sm[i] - 1) / sm[i];
            best = min(best, y * pv[i] + x * pm[i]);
        }
        // also all-big case
        long long yb = (need + sv[i] - 1) / sv[i];
        best = min(best, yb * pv[i]);
        total += best;
