...
class Poly {
   public:
    vmi a;
    Poly() {}
    Poly(const vmi& v) : a(v) { normalize(); }
    void normalize() {
   ...
   private:
    ...
    static vmi poly_sqrt(vmi A, int n) {
        // 基础：n=1 时直接开 a[0]
        if (n == 1) {
            mint r = mod_sqrt(A[0]);
            return {r};
   ...
   public:
    ...
    Poly pow_k(const string& K, int n) const {
        ...
        // 全零 -> 全零
        if (lead >= m) return Poly(vmi(n, mint(0)));
        // 4) 移位递归
        if (lead > 0) {
            ...
            if (ge) return Poly(vmi(n, mint(0)));
            long long kll = 0;
    ...
    static vmi shifted(int m, const vmi& y) {
        int n = sz(y) - 1;
        vmi f(n + 1), invf(n + 1);
        f[0] = 1;
        ...
        vmi v(2 * n + 1);
        rep(i, 0, 2 * n) v[k] = mint::inv(mint(m - n + k));
