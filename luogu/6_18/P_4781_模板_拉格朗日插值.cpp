...
class Poly {
   ...
   public:
    static vector<mint> derivative(const vector<mint>& A) { return poly_derivative(A); }
...
// 在 Poly 类定义之后，work() 之前插入：

// 分治 NTT 多点插值 全局变量
static vector<Poly> seg;
static vector<mint> fval;

// 建树：seg[o] 存 x∈[l..r] 上 ∏(x - xs[i])
void build_seg(int o, int l, int r, const vector<mint>& xs) {
    if (l == r) {
        seg[o] = Poly({-xs[l], 1});
    } else {
        int m = (l + r) >> 1;
        build_seg(o << 1, l, m, xs);
        build_seg(o << 1 | 1, m + 1, r, xs);
        seg[o] = seg[o << 1] * seg[o << 1 | 1];
    }
}

// 分治求 f 在 xs[l..r] 上的值，rem = f % seg[o]
void eval_all(int o, int l, int r, const Poly& f) {
    Poly rem = f % seg[o];
    if (l == r) {
        auto c = rem.coeffs();
        fval[l] = c.empty() ? mint(0) : c[0];
    } else {
        int m = (l + r) >> 1;
        eval_all(o << 1, l, m, rem);
        eval_all(o << 1 | 1, m + 1, r, rem);
    }
}
// 辅助树，用于合并每个叶子的贡献
static vector<Poly> gt;

Poly fast_lagrange_poly(const vector<mint>& xs, const vector<mint>& ys) {
    int n = xs.size();
    // 1. 建树求 T(x)=∏(x−xs[i])
    seg.assign(4 * n, Poly());
    fval.assign(n, mint(0));
    build_seg(1, 0, n - 1, xs);
    Poly T = seg[1];

    // 2. 求 T'
    auto d = Poly::derivative(T.coeffs());
    Poly Tp(d);

    // 3. 计算 prime[i]=T'(xs[i])
    eval_all(1, 0, n - 1, Tp);

    // 4. 计算权重 w[i]=ys[i]/prime[i]
    vector<mint> w(n);
    for (int i = 0; i < n; i++) w[i] = ys[i] / fval[i];

    // 5. 构造合并树：叶子存 w[i]，向上合并时
    //    gt[o] = gt[lc] * seg[rc] + gt[rc] * seg[lc]
    gt.assign(4 * n, Poly());
    ...
    dfs(1, 0, n - 1);
    // gt[1] 就是 ∑ w[i] * ∏_{j≠i}(x−xs[j])，即插值多项式
    return gt[1];
}

// 修改 work() 调用 fast_lagrange_poly 并输出多项式系数
void work() {
    int n;
    cin >> n >> k;
    vector<mint> xs(n), ys(n);
    for (int i = 0; i < n; i++) cin >> xs[i] >> ys[i];
    Poly f = fast_lagrange_poly(xs, ys);
    pt(f.eval(k));
    // auto c = f.coeffs();
    // c.resize(n);  // 不足 n 项的补 0
    // for (int i = 0; i < n; i++) {
    // cout << (c[i]) << (i + 1 == n ? '\n' : ' ');
    // }
}
