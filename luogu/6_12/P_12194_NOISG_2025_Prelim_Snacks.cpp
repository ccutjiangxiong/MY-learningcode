...
using trie = trie<Key, Mapped, Access, pat_trie_tag, trie_prefix_search_node_update>;
...
void work() {
    cin >> n >> m;
    rep(i, 1, n) cin >> a[i], mp[a[i]]++, ans += a[i];
    pt(ans);
    rep(i, 1, m) {
        cin >> x >> y >> z;
        auto it = mp.low(x);
