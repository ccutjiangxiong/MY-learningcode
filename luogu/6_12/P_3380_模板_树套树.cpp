...
void work() {
    ...
    rep(i, 1, n) {
        cin >> y >> x;
        if (y == 1) ss.insert({x, i});
        if (y == 2) {
            auto it = ss.low({x, 0});
            if (it != ss.end() && it->fi == x) ss.erase(it);
        }
        if (y == 3) pt(ss.order_of_key({x, 0}) + 1);
        if (y == 4) pt(ss.find_by_order(x - 1)->fi);
        if (y == 5) pt(prev(ss.low({x, 0}))->fi);
        if (y == 6) pt((ss.low({x + 1, 0})->fi));
    }
}
