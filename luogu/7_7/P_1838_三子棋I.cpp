...
void init(int n) {
    euler(n);
    rep(i, 1, n) if (mu[i]) for (int j = i, k = 1; j <= n; j += i, k++) con[j][mu[i] = 1].pb(k);
    rep(i, 1, n) for (int j = i; j <= n; j += i) d[j].pb(i);
}
vi cyc_me[N];
vi cyclo(int n) {
    if (sz(cyc_me[n])) return cyc_me[n];
    int deg = phi[n];
    vi a(deg + 1, 0);
    a[0] = 1;
    for (int e : con[n][1]) rem(i, deg, e) a[i] -= a[i - e];
    for (int e : con[n][0]) rep(i, e, deg) a[i] += a[i - e];
    if (a[deg] < 0)
        for (int &v : a) v = -v;
    return cyc_me[n] = a;
}
