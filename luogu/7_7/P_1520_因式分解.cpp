...
static Poly cyclo_mem[N];
static bool cyclo_vis[N];

// φₙ(x) = (xⁿ − 1) ÷ ∏_{d|n, d<n} φ_d(x)
Poly cycpoly(int n) {
    if (cyclo_vis[n]) return cyclo_mem[n];
    Poly res;
    if (n == 1) {
        // φ₁(x) = x − 1
        res = Poly(vmi{mint(-1), mint(1)});
    } else {
        // start with xⁿ − 1
        vmi t(n + 1);
        t[0] = mint(-1);
        t[n] = mint(1);
        res = Poly(t);
        // divide out all φ_d for d|n, d<n
        for (int d = 1; d * d <= n; d++) {
            if (n % d) continue;
            if (d < n) res = res / cycpoly(d);
            int e = n / d;
            if (e != d && e < n) res = res / cycpoly(e);
        }
    }
    cyclo_vis[n] = true;
    return cyclo_mem[n] = res;
}
// …existing code…
...
// 多项式 (mint) → 整型系数向量
static vector<ll> to_ll(const vmi& a) {
    vector<ll> r(a.size());
    for (int i = 0; i < (int)a.size(); i++) {
        ll x = (ll)a[i].num;
        if (x > MOD / 2) x -= MOD;  // 还原为负数
        r[i] = x;
    }
    return r;
}
...
// 将系数向量转成题目要求的格式字符串
static string poly2str(const vector<ll>& a) {
    string s;
    int m = a.size();
    for (int i = m - 1; i >= 0; i--) {
        ll c = a[i];
...
void work() {
    ...
    for (int d : ds) {
        auto P = cycpoly(d);
        auto coeff = to_ll(P.a);
        facs.push_back({coeff, poly2str(coeff)});
    }
    // 排序：先次数，再系数绝对值，再符号
    sort(all(facs), [](auto& A, auto& B) {
        int da = (int)A.a.size() - 1, db = (int)B.a.size() - 1;
