...
void work() {
    cin >> n >> m >> k;
    rep(i, 1, m) cin >> x >> y, gr[x][y] = 1;
    // for(int x:g)
    rep(i, 1, k) cin >> x >> y, gr[x][y] = 2;

    rep(i, 1, n) rep(j, 1, n) {
        if (gr[i][j] == 1) {
            rep(u, -1, 1) rep(v, -1, 1) {
                int ni = clamp(u + i, 1ll, n), nj = clamp(j + v, 1ll, n);
                // pt(i, u, j, v, ni, nj);
                if ((!gr[ni][nj])) gr[ni][nj] = 3;
            }
            for (int u : {-2, 0, 2})
                for (int v : {-2, 0, 2}) {
                    int ni = clamp(u + i, 1ll, n), nj = clamp(v + j, 1ll, n);
                    if ((abs(u) + abs(v)) <= 2 && gr[ni][nj] == 0) gr[ni][nj] = 3;
                }
        }
        if (gr[i][j] == 2) {
            rep(u, -2, 2) rep(v, -2, 2) {
    ...
    rep(i, 1, n) rep(j, 1, n) if (!gr[i][j]) ans++;
    cout << ans << endl;
}
