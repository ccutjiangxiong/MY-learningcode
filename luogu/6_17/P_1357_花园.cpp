...
void work() {
    cin >> n >> m >> k;
    lim = 1 << (m - 1);
    vector<vector<int>> M(lim, vector<int>(lim, 0));
    rep(s, 0, lim - 1) {
        int cnt = cnt1(s);
        rep(b, 0, 1) {
            if (cnt + b <= k) {
    ...
    auto mul = [&](auto &A, auto &B) {
        vector<vi> C(lim, vi(lim, 0));
        rep(i, 0, lim - 1) rep(t, 0, lim - 1) if (A[i][t]) {
            int a = A[i][t];
            rep(j, 0, lim - 1)(C[i][j] += a * B[t][j]) %= mod;
        }
        return C;
    };
    vector<vi> R(lim, vi(lim, 0));
    rep(i, 0, lim - 1) R[i][i] = 1;
    int e = n;
    while (e > 0) {
        if (e & 1) R = mul(R, M);
        M = mul(M, M);
