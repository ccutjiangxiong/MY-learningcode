...
int a[555][555], s1[555][555], sa[555][555], ss[555][555];
vii e[N];
...
void work() {
    ...
    rep(i, 1, n - 1) rep(j, 1, m) {
        cin >> x;
        ans += x;
        int id = (i - 1) * m + j, id2 = id + m, id3 = id + n * m;
        add(s, id3, x);
        add(id3, id, inf);
        add(id3, id2, inf);
    }
    rep(i, 1, n - 1) rep(j, 1, m) {
        cin >> x;
        ans += x;
        int id = (i - 1) * m + j, id2 = id + m, id3 = id + n * m * 2;
        add(id3, t, x);
        add(id, id3, inf);
        add(id2, id3, inf);
    }
    rep(i, 1, n) rep(j, 1, m - 1) {
        cin >> x;
        ans += x;
        int id = (i - 1) * m + j, id2 = id + 1, id3 = id + n * m * 3;
        add(s, id3, x);
        add(id3, id, inf);
        add(id3, id2, inf);
    }
    rep(i, 1, n) rep(j, 1, m - 1) {
        cin >> x;
        int id = (i - 1) * m + j, id2 = id + 1, id3 = id + n * m * 4;
        ans += x;
        add(id3, t, x);
        add(id, id3, inf);
        add(id2, id3, inf);
