...
struct Seg {
    Node* root;
    vector<ll>& w;
    Seg(vi& a) : w(a) { root = build(1, sz(w) - 1); }
    ...
    void push(Node* p) {
        if (p->add) {
            for (Node* c : {p->lc, p->rc}) {
                c->sum += p->add * (c->r - c->l + 1);
                c->add += p->add;
            }
            p->add = 0;
        }
    ...
    void update(Node* p, int l, int r, ll v) {
        if (l <= p->l && p->r <= r) {
            p->sum += v * (p->r - p->l + 1);
            p->add += v;
            return;
        ...
        push(p);
        int m = (p->l + p->r) >> 1;
        if (l <= m) update(p->lc, l, r, v);
        if (r > m) update(p->rc, l, r, v);
        p->sum = p->lc->sum + p->rc->sum;
    }
    ...
    ll query(Node* p, int l, int r) {
        ...
        int m = (p->l + p->r) >> 1;
        ll ans = 0;
        if (l <= m) ans += query(p->lc, l, r);
        if (r > m) ans += query(p->rc, l, r);
        return ans;
    }

    void update(int l, int r, ll v) { update(root, l, r, v); }
