...
struct mint {
    ...
    friend std::istream& operator>>(std::istream& in, mint& x) { return in >> x.num, in; }
    constexpr mint& operator+=(const mint& o) noexcept {
        return num = ((ill)num + o.num) % Mod, *this;
    }
    constexpr mint& operator-=(const mint& o) noexcept { return *this += -o; }
    constexpr mint& operator*=(const mint& o) noexcept {
        return num = (ill)num * o.num % Mod, *this;
    }
    constexpr mint& operator/=(const mint& o) noexcept { return *this *= inv(o); }
    friend constexpr mint operator^(mint a, int e) noexcept { return fastPow(a, e); }
    constexpr mint operator-() const noexcept { return (Mod - num) % Mod; }
    friend constexpr auto operator<=>(const mint& a, const mint& b) noexcept = default;
