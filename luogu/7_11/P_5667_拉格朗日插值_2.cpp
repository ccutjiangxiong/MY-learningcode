...
struct Mont {
    ...
    friend istream& operator>>(istream& is, Mont& m) {
        int x;
        is >> x, m = x;
        return is;
    }
...
vmi Shift(const vmi& y, int m) {
    ...
    invb[2 * n] = mint::inv(tmp[2 * n]), v[0] = mint::inv(ba);
...
inline int Fac(int n) {
    int p = 0, cur = 0, s = sqrt(n);
    mint invs = mint::inv(s);
    for (int i = s; i > 1; i >>= 1) stk[++cur] = i;
    c.resize(2);
    c[0] = 1, c[1] = s + 1;
    for (int l = stk[cur]; cur > p; l = stk[--cur]) {
        d = Shift(c, (l >> 1) + 1);
        // break;
        c.resize(2 * sz(c));

        for (int i = 0; i < sz(d); ++i) c[sz(d) + i] = d[i];
        d = Shift(c, (int)(invs * mint(l >> 1)));
