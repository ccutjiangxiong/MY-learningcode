...
void work() {
    cin >> k >> n;
    // collect answers in a vector of triples
    vector<array<ll, 3>> ans;
    ans.reserve(n);

    // TODO: replace the body of this loop with the actual construction
    // of your n distinct solutions for
    //    a^2 + b^2 + c^2 = k(ab + bc + ca) + 1
    //
    // here's just a placeholder that compiles
    for (ll i = 1; i <= n; i++) {
        ll a = 1;
        ll b = k * i + 1;
        // solve c from the quadratic:
        //   c^2 - k(a+b)c + (a^2 + b^2 - kab - 1) = 0
        // (here we just set c = b)
        ll c = b;
        ans.push_back({a, b, c});
    }
