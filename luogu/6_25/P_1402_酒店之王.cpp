...
void work() {
    int n, p, q;
    cin >> n >> p >> q;
    rep(i, 1, n) rep(j, 1, p) cin >> gr[0][i][j];
    rep(i, 1, n) rep(j, 1, q) cin >> gr[1][i][j];
    int s = 0, t = 2 * n + p + q + 1;
    rep(i, 1, p) add(s, i, 1, 0);
    rep(i, 1, p) rep(j, 1, n) if (gr[0][j][i]) add(i, j + p, 1, 0);
    rep(i, 1, n) add(i + p, i + p + n, 1, 0);
    rep(i, 1, n) rep(j, 1, q) if (gr[1][i][j]) add(i + p + n, n + p + n + j, 1, 0);
    rep(i, 1, q) add(n + p + n + i, t, 1, 0);
    pt(dinic(s, t));
}
