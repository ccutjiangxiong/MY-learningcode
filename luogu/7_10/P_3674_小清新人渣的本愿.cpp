...
int ti = 1, n, m, k, a[N], b[N], c[N], res, tot, x, y, z;
int f1[N], f2[N], g1[N], g2[N];
vi e[N];
int B;
const int C = 1e5;
bitset<C + 1> bs, ans, bs2;

int cnt[N];
void add(int x) {
    // pt('a', cnt[x]);
    if (++cnt[x] == 1) bs[x] = bs2[C - x] = 1;
    // pt('b', cnt[x]);
    if (cnt[x] != 1 && bs[x] == 0) pt(x, cnt[x], bs[x]);
}
void del(int x) {
    if (--cnt[x] == 0) bs[x] = bs2[C - x] = 0;
}

int ask(int op, int x) {
    if (op == 1) return ((bs << x) & bs).any();
    if (op == 2) return (bs & (bs2 >> (C - x))).any();
    if (op == 3) {
        for (int i = 1; i * i <= x; i++) {
            if (x % i == 0 && bs[i] && bs[x / i]) return true;
        }
    }
    return false;
}
