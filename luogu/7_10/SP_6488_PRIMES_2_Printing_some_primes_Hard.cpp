...
vi e[N];
const int MAXP = 50000000;
const int BLK = 510510;  // = 2*3*5*7*11*13*17
const int W = BLK >> 6, M0 = 7, K0 = 4444;
#define SET(b, i) (b[(i) >> 6] |= 1ull << ((i) & 63))
typedef unsigned char u8;
int pr[MAXP + 7];               // 存储素数
uint64_t b0[W + 1], b1[W + 1];  // 预处理块和当前块位图
u8 flag[BLK + 7];               // 小区间筛选标记
int cnt, id, total;
void Prime() {
    flag[0] = flag[1] = 1;
    SET(b0, 0);
    ...
    for (int i = 2; i <= BLK; ++i) {
        ...
        for (int j = 1; j <= cnt && i * pr[j] <= BLK; ++j) {
            int t = i * pr[j];
            flag[t] = 1;
            if (j <= M0) SET(b0, t);
            if (i % pr[j] == 0) break;
...
void work() {
    Prime();
    // for (int i = 1; i <= 1e9; i += 500) pt(pr[i]);
    pt(total, pr[total]);
}
