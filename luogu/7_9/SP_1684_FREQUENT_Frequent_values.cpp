...
vi e[N];
int cnt[N], sum,ml,mr,mx;
const int OFFSET = 100000;   
int freqVal[2 * OFFSET + 5], freqCnt[N], curMax;
void add(int x) {
    int v = a[x] + OFFSET;
    int f = freqVal[v]++;
    freqCnt[f]--;
    freqCnt[f + 1]++;
    curMax = max(curMax, f + 1);
}
void del(int x) {
    int v = a[x] + OFFSET;
    int f = freqVal[v]--;
