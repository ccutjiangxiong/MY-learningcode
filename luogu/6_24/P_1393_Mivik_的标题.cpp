...
void work() {
    cin >> n >> m >> k;
    // 读名字并做 KMP，求出所有 border lengths 存在 vector<int> b;
    string s;
    rep(i, 1, k) {
        cin >> a[i];
        s.push_back(char('a' + (a[i] % m)));
    }
    kmp(s);
    vi b;
    for (int p = ne[k]; p > 0; p = ne[p]) b.pb(p);
    // 构造 D(x)=1 - m x + sum_{p in b} x^{k-p}
    vmi d(k + 1);
    d[0] = 1;
    d[1] = mint(-m);
    for (int p : b) {
    ...
    Poly D(d);
    Poly G = D.inv(n + 1);
    mint gn = (n < (int)G.a.size() ? G.a[n] : mint(0));
    // ans = 1 - g_n / m^n
    mint invm = mint::inv(mint(m));
    mint ans = (mint(1) - gn * mint::fastPow(invm, n));
    cout << ans << "\n";
}
