...
void work() {
    ...
    sort(ev.begin(), ev.end(), [](auto& l, auto& r) { return l.first < r.first; });
    ...
    while (idx < m) {
        int v = ev[idx].first;
        while (idx < m && ev[idx].first == v) {
            int i = ev[idx].second;
            if (cnt[i] == 0) {
                cnt[i] = 1;
                zero--;
            } else {
                C = C * mint(cnt[i] + 1) * (1 / mint(cnt[i]));
                cnt[i]++;
            }
            idx++;
        }
        mint curP = zero > 0 ? mint(0) : C * inv6N;
        mint delta = curP - prevP;
        ans += delta * v;
        prevP = curP;
    }
    cout << ans << "\n";
