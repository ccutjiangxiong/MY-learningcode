...
void work() {
    cin >> n >> k >> m;
    rep(i, 0, 1) rep(j, 0, k) dp[i][j] = -1;
    dp[0][k] = m;
    rep(i, 1, n) {
        cin >> a[i] >> b[i];
        int cur = i & 1, pre = cur ^ 1;
        rep(j, 0, k) dp[cur][j] = -1;
        rep(h, 0, k) {
            if (dp[pre][h] < 0) continue;
            if (h >= a[i]) {
                dp[cur][h - a[i]] = max(dp[cur][h - a[i]], dp[pre][h]);
            }
            if (dp[pre][h] >= b[i]) {
                dp[cur][h] = max(dp[cur][h], dp[pre][h] - b[i]);
            }
        }
        bool ok = false;
        rep(h, 0, k) if (dp[cur][h] >= 0) {
            ok = true;
            break;
        }
        if (!ok) break;
