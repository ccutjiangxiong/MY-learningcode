...
int w[N];
inline int &Max(int a) { return tr[a].sum; }
...
int query(int p, int x, int y, int &u) {
    // pt(p);
    if (x <= l(p) && y >= r(p)) return u = tr[p].u, Max(p);
...
void work() {
    ...
    rep(i, 1, n) {
        int u, v;
        int mx = query(1, i, min(n, i + m), u);
        int x = query(1, i, i, v), y = query(1, u, u, v);
        if (u != i) update(1, u, 0), m -= (u - i);
    }
    rep(i, 1, n) cout << query(1, i, i, k) << ' ';
}
